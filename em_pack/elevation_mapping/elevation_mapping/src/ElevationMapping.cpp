...
void ElevationMapping::setupSubscribers() {  // Handle deprecated point_cloud_topic and input_sources configuration.
  ...
  if (!configuredInputSources && hasDeprecatedPointcloudTopic) {
    pointCloudSubscriber_ = nodeHandle_.subscribe<sensor_msgs::PointCloud2>(
        parameters.pointCloudTopic_, 1, [&](const auto& msg) { pointCloudCallback(msg, true, sensorProcessor_); });       // 回调函数：将当前所有变量作为引用传入为msg，之后传入pointCloudCallback
  }
  // 若有input configure
  if (configuredInputSources) {
    inputSources_.registerCallbacks(*this, make_pair("pointcloud", &ElevationMapping::pointCloudCallback));               // 注册回调：传入this指针，pointCloudCallback作为引用和“pointcloud”作为pair传入
  }
  /******************************************************'/pose'***************************************************************/
  // 若robotPoseTopic_为空
  if (!parameters.robotPoseTopic_.empty()) {
    robotPoseSubscriber_.subscribe(nodeHandle_, parameters.robotPoseTopic_, 1);                                           // 订阅robot pose topic
    robotPoseCache_.connectInput(robotPoseSubscriber_);                                                                   // 把cache和subscribe链接起来
    robotPoseCache_.setCacheSize(parameters.robotPoseCacheSize_);                                                         // 设置缓存长度
  } else {
...
void ElevationMapping::pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& pointCloudMsg, bool publishPointCloud,
  ...
  const Parameters parameters{parameters_.getData()};
  ROS_DEBUG("Processing data from: %s", pointCloudMsg->header.frame_id.c_str());        // 检查当前的frame_id
  if (!parameters.updatesEnabled_) {
    ...
    if (publishPointCloud) {
      map_.setTimestamp(ros::Time::now());                                              // 设置当前时间戳
